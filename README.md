...
- **Server Statistics page** (`/stats`, in Players ▾ submenu) — public community-pride dashboard: 4 hero KPIs (real accounts / characters / online now / guilds), faction-balance split bar, class distribution (class-coloured horizontal bars), race distribution, level histogram (1-9 → 90 in 10-level buckets), activity tiles (today / this week / this month / older by last login), and a 30-day new-registrations SVG line chart. Pure CSS + inline SVG — no external chart library. Repack-defensive: each section silently hides when its underlying query fails. Bot accounts (TrinityCore wow-bot fork) are excluded from the account count.
- **Two-Factor Authentication (TOTP)** — opt-in second factor login at `/account_2fa` (also surfaced on the dashboard Account & Security tab). Pairs with any TOTP authenticator (Google Authenticator, Authy, 1Password, Bitwarden, Aegis…) via a QR code or manual secret entry. 8 single-use backup codes are generated on enrolment (and on regenerate) — shown once, stored as `sha256` hashes. Login becomes two-stage when 2FA is on (password verifies → 6-digit code prompt; pending state expires after 5 min). **Password recovery and password change also require a 2FA code** when 2FA is enabled — closes the "own the email → bypass 2FA" hole and the "session-hijack → silent password change" hole. Pure RFC 6238 implementation (no Composer dependency).
- **Change email address** — new `/change_email` flow. User enters the new address + current password (+ a 2FA code if 2FA is on). A confirmation link is mailed to the **CURRENT** address; clicking it commits. Same security shape as password recovery: even a hijacked session can't silently rotate the email because the legitimate owner of the old address still has to approve. New `web_email_changes` table.
- **Dashboard reorganised into 3 tabs** — the dashboard had quietly grown 13 sections and 1100+ lines of accumulated panels. Now structured as **Overview** (stat cards, playtime reward, playtime distribution chart, vote sites, most-played callout), **Characters** (full character grid), and **Account & Security** (account info + change email/password + 2FA status + login history, last 10). Tab state persists in the URL hash so reload returns to the same view. Redundant "Quick Links" panel removed (its links live in the Players ▾ navbar dropdown now). Admin-Panel link removed from user dashboard (GMs already have it in the user menu).
- **Public JSON API** at `/api` — three endpoints for community Discord bots / streamer overlays / external trackers: `/api/online` (current roster + faction split), `/api/leaderboards/<2v2|3v3|rbg>` (current-season top-N), `/api/character/<name>` (single profile). Read-only, no auth, CORS `*`, modest server-side caching (20–60s). Hit `/api` with no suffix for the HTML reference page with curl + JS examples and an ID map (class/race/gender). Linked from the footer "Developer API" entry.
- **Discord widget on the homepage** — when `config.discord.server_id` is set, the homepage renders a live Discord block right under the hero: server name, presence count ("X members online now"), Join button. Server-side fetch of Discord's public `widget.json`, cached ~60s in `web_site_settings`, fail-silent (block hides on any error or when the widget isn't enabled in Discord's Server Settings → Widget). No external embed iframe — pure HTML/CSS so it themes with the rest of the site.
