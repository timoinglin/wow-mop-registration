...
- **Armory Talents — in-game-style 6×3 grid** (when the class's MoP talent map is filled — Warrior is verified; the other 10 classes fall back to the existing Wowhead chip cloud until their IDs are filled in `includes/wow_talents_mop.php`). Chosen talent glows in the class colour; unchosen options dimmed. Mobile: 1-column stack.
