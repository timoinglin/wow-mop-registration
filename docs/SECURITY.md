...
- News image uploads (`/news_image`):
  ...
  - Every successful upload is recorded in `web_admin_audit_log`
...
- Forum **bans are forum-only** (separate `web_forum_bans` table). They don't touch `account_banned` and don't affect game login or the ticket system. GM 9+ accounts cannot be added to `web_forum_bans` — admin-vs-admin lockout is impossible by construction.
...
- Admin actions are logged to `web_admin_audit_log` with IP, timestamp, and details
